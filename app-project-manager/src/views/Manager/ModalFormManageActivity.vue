  ...
  <PrimerModal v-if="show" :displayFooter="true" :displayHeader="false">
    <template v-slot:body>
      <div class="container-md">
        ...
        <p class="note">Current Progress {{ activity.progress.toFixed(0) }}%</p>
        <span class="Progress Progress--small">
          <span class="Progress-item bg-green" :style="{ width: activity.progress + '%' }"> </span>
        </span>
        ...
        <div class="Box Box--condensed">
          <!-- Resources List  -->
          <div class="Box-row" v-for="rsc in activity.resources" :key="rsc.title">
            <p class="d-block">
              {{ rsc.hasOwnProperty("assignee") ? "⌛ Personell Resource '" : "💰 External Cost Resource '"
              }}<b>{{ rsc.title }}</b
              >'
            </p>
        ...
        <!-- DocumentRefs list -->
        <ListDocuments :documents="activity.documents" @removeDocument="removeDocument" />

        <!-- 'Add' Documents buttons -->
        <button class="btn mt-4" type="button" @click="activity.addDocuments()">
    ...
    <template v-slot:footer>
      <div class="container-md">
        ...
        <!-- Finish button -->
        <button class="btn btn-primary mr-2" type="button" @click="finish">
          <span>Finish</span>
        </button>
      </div>
    </template>
