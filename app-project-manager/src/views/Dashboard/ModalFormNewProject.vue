  ...
  <PrimerModal v-if="show" :displayFooter="true" :displayHeader="false">
    <template v-slot:body>
      <div class="container-md">
        ...
        <div class="Subhead hx_Subhead--responsive mb-5">
          <h1 class="Subhead-heading">Create a new project</h1>
        </div>
    ...
    <template v-slot:footer>
      <div class="container-md">
        ...
        <div class="mt-2">
          <button class="btn btn-primary mr-2" type="button" @click="saveNewProject">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const approachModels: ComputedRef<Array<ISelectItem>> = computed(() => {
      ...
      const mapped: Array<ISelectItem> = models.map((a) => {
        return { name: a.title, payload: a } as ISelectItem;
      });
    ...
    const availableProjectLeads: ComputedRef<Array<ISelectItem>> = computed(() => {
      ...
      const mapped: Array<ISelectItem> = projectLeaders.map((pl) => {
        return { name: pl.fullName, payload: pl } as ISelectItem;
      });
    ...
    const saveNewProject = () => {
      ...
      if (title && id && model && projectLead && priority && startDate && description) {
        const newProject: Project = new Project(title, id, model, description, startDate, priority, projectLead);

        loadingbar.start();

        store
          .dispatch(ActionTypes.setProjectToBePlanned, newProject)
          .then((res: string) => {
            emit("done");
